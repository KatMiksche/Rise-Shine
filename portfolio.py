...
class prtfl:
    ...
    def buy(self,cursor,wallet, ticker,volume):
        ...
        if price>wallet.CurrentValue(cursor):
            ...
            if ticker in self.hold['Ticker']:
                ...
                data = [ticker, volume, self.id]
                cursor.execute('INSERT INTO hold VALUES(%s, %s, %s)',data)
            self.load_hold(cursor)
            self.load_currentvalue(cursor)
    ...
    def update(self,cursor):
        if self.historic_value=='': self.SQL_load(cursor)
        if self.hold=='': self.SQL_load(cursor)
        ...
        if difference.days>1:
            ...
            print(historic_records)
            days=historic_records.loc[historic_records['timestamp'] == str(self.historic_value[-1][0])].index[0]
            days=days-1
